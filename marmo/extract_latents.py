...
def parse_args():
    p = argparse.ArgumentParser(description="Extract FOND latents aligned to BackImage V1 responses")
    ...
    p.add_argument("--max-windows-per-split", type=int, default=512)
...
def main():
    args = parse_args()
    model, cfg, extra = load_model_from_checkpoint(args.checkpoint, map_location="cpu")
